...
export function PlanDetail({ planName, onClose }: Props) {
  ...
  const [dataSub, setDataSub] = useState<DataSub>('daily')
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-100 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        {activeTab === 'data' && (
          <div className="p-4 space-y-3">
            {/* Section head with sub-tabs */}
            <div className="bg-white rounded-xl shadow-sm overflow-hidden">
              <div className="px-4 py-3 font-bold text-sm border-b border-gray-100 flex items-center gap-3 flex-wrap">
                📈 推广效果数据
                <div className="ml-auto flex rounded-lg overflow-hidden border border-gray-200">
                  <button onClick={() => setDataSub('daily')}
                    className={`px-3 py-1 text-xs font-semibold ${dataSub === 'daily' ? 'bg-indigo-800 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
                    📅 日数据（{dateRange}天）
                  </button>
                  <button onClick={() => setDataSub('hourly')}
                    className={`px-3 py-1 text-xs font-semibold border-l border-gray-200 ${dataSub === 'hourly' ? 'bg-indigo-800 text-white' : 'bg-white text-gray-600 hover:bg-gray-50'}`}>
                    ⏰ 时数据（今日）
                  </button>
                </div>
              </div>
              ...
              {dataSub === 'daily' && (
                <div className="p-4">
                  {/* Controls */}
                  <div className="flex items-center gap-2 flex-wrap mb-3 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2">
                    <span className="text-xs font-semibold text-gray-500">时间范围</span>
                    <div className="flex gap-1">
                      {([7, 14, 30] as const).map(d => (
                        <button key={d} onClick={() => setDateRange(d)}
                          className={`px-2.5 py-1 rounded-full text-xs font-semibold border transition-all
                            ${dateRange === d ? 'bg-indigo-800 text-white border-indigo-800' : 'bg-white text-gray-500 border-gray-200 hover:border-indigo-400 hover:text-indigo-800'}`}>
                          {d}天
                        </button>
                      ))}
                    </div>
                    <span className="text-gray-300 px-1">|</span>
                    <span className="text-xs font-semibold text-gray-500">聚合</span>
                    {(['day', 'week'] as const).map(a => (
                      <button key={a} onClick={() => setAgg(a)}
                        className={`px-2.5 py-1 rounded-full text-xs font-semibold border transition-all
                          ${agg === a ? 'bg-indigo-800 text-white border-indigo-800' : 'bg-white text-gray-500 border-gray-200 hover:border-indigo-400 hover:text-indigo-800'}`}>
                        {a === 'day' ? '日' : '周'}
                      </button>
                    ))}
                    <span className="text-gray-300 px-1">|</span>
                    <span className="text-xs font-semibold text-gray-500">指标</span>
                    {(['spend', 'rev', 'febi', 'roi'] as const).map(k => (
                      <button key={k} onClick={() => setMetricKey(k)}
                        className={`px-2.5 py-1 rounded-full text-xs font-semibold border transition-all
                          ${metricKey === k ? 'bg-indigo-800 text-white border-indigo-800' : 'bg-white text-gray-500 border-gray-200 hover:border-indigo-400 hover:text-indigo-800'}`}>
                  ...
                  <div className="flex gap-4 text-xs text-gray-500 mb-2">
                    <span><span className="inline-block w-3 h-3 rounded-sm mr-1" style={{ background: 'rgba(21,101,192,.6)' }} />花费（柱）</span>
                    <span><span className="text-red-700">■</span> 红区</span>
                    <span><span className="text-yellow-600">■</span> 黄区</span>
                    <span><span className="text-green-700">■</span> 绿区</span>
                  </div>
                  {/* Chart */}
                  <div className="h-52 mb-3">
                    <ResponsiveContainer width="100%" height="100%">
                      <ComposedChart data={dailyData}>
                        ...
                        <Bar yAxisId="left" dataKey="spend" fill="rgba(21,101,192,.6)" name="花费" radius={[2, 2, 0, 0]} />
                        {metricKey !== 'spend' && (
                          <Line yAxisId="right" type="monotone" dataKey={metricKey} stroke="#283593" strokeWidth={2} dot={{ r: 2 }} name={metricKey === 'rev' ? '成交额' : metricKey === 'febi' ? '费比' : 'ROI'} />
                        )}
                        {metricKey === 'roi' && (
                          <>
                            <ReferenceLine yAxisId="right" y={targetROI} stroke="#2e7d32" strokeDasharray="4 2" label={{ value: `目标ROI ${targetROI}`, fill: '#2e7d32', fontSize: 8 }} />
                            <ReferenceLine yAxisId="right" y={stopLossROI} stroke="#c62828" strokeDasharray="4 2" label={{ value: `止损ROI ${stopLossROI}`, fill: '#c62828', fontSize: 8 }} />
                          </>
                        )}
                        {metricKey === 'febi' && (
                          <>
                            <ReferenceLine yAxisId="right" y={plan.gross * 100} stroke="#c62828" strokeDasharray="4 2" label={{ value: `止损 ${(plan.gross * 100).toFixed(0)}%`, fill: '#c62828', fontSize: 8 }} />
                            <ReferenceLine yAxisId="right" y={plan.gross * 100 - 10} stroke="#2e7d32" strokeDasharray="4 2" label={{ value: `目标 ${(plan.gross * 100 - 10).toFixed(0)}%`, fill: '#2e7d32', fontSize: 8 }} />
                          </>
                        )}
                      </ComposedChart>
                    </ResponsiveContainer>
                    ...
                    <table className="w-full text-xs border-collapse">
                      <thead className="sticky top-0 bg-gray-50 z-10">
                        <tr className="border-b-2 border-gray-200">
                          {['日期', '花费', '成交额', 'ROI', '费比', '订单数', '转化率', '点击数', '展示数', '点击率', '均CPC', '收藏', '加购', '规则/动作'].map(h => (
                            <th key={h} className="px-2 py-1.5 text-right first:text-left font-bold text-gray-500 whitespace-nowrap">{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {[...dailyData].reverse().map((d, i) => {
                          const rowCls = d.atype === 'red' ? 'bg-red-50' : d.atype === 'yellow' ? 'bg-yellow-50' : d.atype === 'green' ? 'bg-green-50/30' : ''
                          ...
                          return (
                            <tr key={i} className={`border-b border-gray-100 hover:brightness-95 ${rowCls}`}>
                              ...
                              <td className="px-2 py-1.5 text-right text-gray-500">{d.favs != null ? d.favs : '—'}</td>
                              <td className="px-2 py-1.5 text-right text-gray-500">{d.addcart != null ? d.addcart : '—'}</td>
                              <td className="px-2 py-1.5 text-left">
                                {d.rule && d.rule !== '—' && (
                                  <span className="text-xs font-bold px-1.5 py-0.5 rounded mr-1 bg-yellow-100 text-yellow-700">{d.rule}</span>
                                )}
                ...
                <div className="p-4">
                  <div className="flex items-center gap-4 mb-3 flex-wrap">
                    <div className="flex gap-3 text-xs text-gray-500">
                      <span><span className="inline-block w-3 h-3 rounded-sm mr-1 align-middle" style={{ background: 'rgba(21,101,192,.6)' }} />小时花费</span>
                      <span><span className="inline-block w-4 border-t-2 border-dashed border-red-700 mr-1 align-middle" />动态阈值</span>
                    </div>
                    <div className="ml-auto text-xs text-gray-500">
                      止损费比: <strong className="text-red-700">{(plan.gross * 100).toFixed(1)}%</strong>
                      &nbsp;&nbsp;目标费比: <strong className="text-green-700">{(plan.gross * 100 - 10).toFixed(1)}%</strong>
                    </div>
                  </div>
                  {/* Hourly chart */}
                  <div className="h-48 mb-3">
                    <ResponsiveContainer width="100%" height="100%">
                      <ComposedChart data={hourlyData}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                        <XAxis dataKey="h" tickFormatter={(h: number) => `${h}:00`} tick={{ fontSize: 9 }} />
                        <YAxis yAxisId="left" tick={{ fontSize: 9 }} />
                        <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 9 }} />
                        <Tooltip formatter={(v, name) => [name === '费比' ? String(v) + '%' : v, name as string]} />
                        <Bar yAxisId="left" dataKey="spend" fill="rgba(21,101,192,.6)" name="花费" radius={[2, 2, 0, 0]} />
                        <Line yAxisId="left" type="monotone" dataKey="threshold" stroke="#c62828" strokeWidth={1.5} strokeDasharray="5 3" dot={false} name="动态阈值" />
                        <Line yAxisId="right" type="monotone" dataKey="febi" stroke="#f57f17" strokeWidth={2} dot={{ r: 3 }} name="费比" />
                        <ReferenceLine yAxisId="right" y={plan.gross * 100} stroke="#c62828" strokeDasharray="4 2" />
                        <ReferenceLine yAxisId="right" y={plan.gross * 100 - 10} stroke="#2e7d32" strokeDasharray="4 2" />
                      </ComposedChart>
                    </ResponsiveContainer>
                  </div>
                  {/* Hourly table */}
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs border-collapse">
                      <thead className="sticky top-0 bg-gray-50 z-10">
                        <tr className="border-b-2 border-gray-200">
                          {['时段', '展示数', '点击数', '点击率', '均CPC', '花费', '阈值', '成交额', 'ROI', '费比', '成交笔数', '转化率', '收藏', '加购', '规则触发/执行动作'].map(h => (
                            <th key={h} className="px-2 py-1.5 text-right first:text-left font-bold text-gray-500 whitespace-nowrap">{h}</th>
                          ))}
                        </tr>
                      </thead>
